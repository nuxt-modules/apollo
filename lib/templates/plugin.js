...
export default (ctx, inject) => {
  ...
  <% Object.keys(options.clientConfigs).forEach((key) => { %>
      ...
      const authFunction = eval(<%= typeof options.clientConfigs[key].getAuth === 'function' ? options.clientConfigs[key].getAuth : ''  %>)
      const getAuth = typeof authFunction === 'function' ?  () => authFunction.call() : () => {
          let token
          if(process.server){
