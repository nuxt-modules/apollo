...
module.exports = function (moduleOptions) {
  ...
  Object.keys(clientConfigs).forEach((key) => {
    ...
    if (typeof clientConfig !== 'object') {
      if (typeof clientConfig !== 'string' || (typeof clientConfig === 'string' && /^https?:\/\//.test(clientConfig))) {
        throw new Error(`[Apollo module] Client configuration "${key}" should be an object or a path to an exported Apollo Client config.`)
      }
    } else {
      if (typeof clientConfig.httpEndpoint !== 'string' || (typeof clientConfig.httpEndpoint === 'string' && /^https?:\/\//.test(clientConfig))) {
        if (typeof clientConfig.link !== 'string' || (typeof clientConfig.link === 'string' && /^https?:\/\//.test(clientConfig))) {
